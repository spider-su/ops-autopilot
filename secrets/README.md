...
the Argo CD repo-server decryption tooling. The pinned Argo CD installation base and the repo-server
patch are rendered by the `argocd-infrastructure` Application. Back the key up in an
offline/password-manager location; the cluster copy alone is not sufficient after cluster loss.
